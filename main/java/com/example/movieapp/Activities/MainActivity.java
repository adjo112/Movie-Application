...
public class MainActivity extends AppCompatActivity {
    ...
    public List<Movie> getMovies(String searchTerm) {
        movieList.clear();
       JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET,Constants.URL_LEFT+searchTerm+
                   Constants.URL_RIGHT+Constants.API_KEY,null,new Response.Listener<JSONObject>() {
               @Override
           public void onResponse(JSONObject response) {
               try{
                   ...
                   for (int i = 0; i < moviesArray.length(); i++) {
                       ...
                       Log.d("Movies",movie.getYear());
                       Log.d("Movies",movie.getMovieType());
                       Log.d("Movies",movie.getImdbId());
